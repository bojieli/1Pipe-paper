...
For the switch, \textit{e.g.,} Arista \RED{model number TBA} without a reconfigurable switching chip, we can implement our in-network processing logics using the switch CPU. 
...
Because the CPU cannot process every data packet, we follow the principle of separating control from data\RED{plane or packet?}.
The control consists of beacon packets carrying timestamp information. \RED{How about data plane?}As shown in Figure~\ref{fig:commodity}, data packets are forwarded by the switching chip as if there were no \sys service, and stored in reorder buffer on end-host receivers.
