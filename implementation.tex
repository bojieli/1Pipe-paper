...
\begin{table}[t]
	...
	\begin{tabular}{l|l}
		...
		\hline
		$T, M$ = TentativeRecv() & Receive a message $M$ at timestamp $T$ in order but may be recalled. \\
		\hline
		RecallCallback($f$($T, M$)) & Register callback $f$ to recall a tentatively received message $M$ at $T$. \\
		\hline
		$T, M$ = ReliableRecv() & Receive a message $M$ reliably in order at timestamp $T$. \\
		\hline
		\hline
		ID = Init(IsHostOrSwitch) & Initialize \sys and get a node ID. \\
		\hline
		Connect(NeighborID) & Connect to a neighbor switch or host. \\
		\hline
		Disconnect(NeighborID) & Disconnect from a neighbor switch or host. \\
		\hline
		FailureCallback($f$(ID, $T$)) & Register callback $f$ for failure notification of ID at timestamp $T$. \\
		\hline
