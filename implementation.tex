...
%However, with a standard RDMA NIC, 
...
\revise{In a large network, the controller can be distributed to a cluster, each of which communicates with a portion of processes.}
...
\revise{The expected delay of best effort \sys{} is base delay + (beacon interval + clock skew).}
...
\revise{So, the expected delay is base delay + (switch CPU processing delay $\times$ number of hops + beacon interval + clock skew).}
...
If the switch vendor does not expose access interfaces to switch CPUs, we can offload the beacon processing to end hosts. \revise{We designate an \emph{end-host representative} for each network switch. The challenge is that best effort \sys{} requires the barrier timestamp to be the lower bound of future message timestamp on a network link $L$. So, beacons with barrier timestamps on $L$ must pass through $L$.\footnote{Reliable \sys{} does not require this property.}}
...
\revise{The expected delay is base delay + ((RTT between switch and host + host processing delay) $\times$ number of hops + beacon interval + clock skew).}
\revise{Because CPUs on end hosts may have shorter processing delay (via DPDK) than switch CPU (via OS IP stack), host delegation may have shorter overall delay. This is why we use host delegation for evaluations in Sec.\ref{sec:evaluation}.}
