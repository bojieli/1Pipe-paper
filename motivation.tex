...
Best effort \sys{} detects lost packets via end-to-end ACK, but does not retransmit them. \revise{Modern data centers typically have low network utilization and deploy advanced congestion control mechanisms~\cite{kumar2020swift,hu2020aeolus,mittal2015timely,zhu2015congestion,alizadeh2010data}. Hence, packet losses are rare~\wei{cite pingmesh paper to show a concrete number here} and most of losses are due to network failures and packet corruptions. Packet corruption rate should be below $10^{-8}$ according to IEEE 802.3 standard, and links with corruption rate higher than $10^{-6}$ are considered to be faulty~\cite{zhuo2017understanding}.}
...
\revise{Production data centers provide multiple paths between two nodes~\cite{greenberg2009vl2,fattree}.} Due to different delays of different paths, several categories of ordering hazards~\cite{gharachorloo1990memory,sewell2010x86}\wei{fix the citation here. it is shown as ?} may take place (Figure~\ref{fig:ordering}).
