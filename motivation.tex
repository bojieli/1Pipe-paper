...
So, data center applications can assume that best effort \sys{} is almost reliable but should use \textit{onepipe\_send\_fail\_callback} to detect lost packets due to packet corruption or failure of network or remote process. \revise{Loss recovery is up to the application.}
...
If we use reliable \sys{}, \revise{objects no longer need versioning, but the end-to-end latency increases from 2.5 to 3.5 RTTs}. However, $A$ can still send messages in pipeline, and have much higher throughput than the traditional way.
