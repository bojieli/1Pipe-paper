...
\parab{Validity.} If the sender and all receivers $R_i$ of a message scattering $S$ have no failure, and the network has no permanent failure, each message $M_i \in S$ is eventually delivered by $R_i$. No message is delivered more than once.

\parab{Atomicity.} For any pair messages $M_x$ and $M_y$ in a same message scattering, either $R_x$ deliver $M_x$ and $R_y$ deliver $M_y$, or $R_x$ does not deliver $M_x$ and $R_y$ does not deliver $M_y$.

\parab{Total Order.}There is a total order among message scatterings. For each pair of message scatterings $S_i$ and $S_j$ with at least one overlapping receiver $R$, $R$ is delivered message $M_i \in S_i$ before $M_j \in S_j$ if and only if $S_i$ is ordered before $S_j$.

\parab{FIFO Order.}For each pair of message scatterings $S_i$ and $S_j$ with a same sender, $S_i$ is sent before $S_j$ if and only if $S_i$ is ordered before $S_j$.

\parab{Causality.}The total order of message scatterings preserves causality. For each pair of message scatterings $S_i$ and $S_j$, if the sender of $S_j$ is one receiver of $S_i$, and $S_j$ is sent after receiving $M_i \in S_i$, then $S_i$ is ordered before $S_j$.
...
There has been extensive research in total order communication, mostly providing a multicast or broadcast primitive.
