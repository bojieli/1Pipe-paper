...
Host $A$ writes data to another host $O$, then sends a notification to host $B$. Send can be considered a kind of write operation.
When $B$ receives the notification, it issues read command to $O$, but may not get the data due to reordering between $A$'s write to $O$ and $B$.
...
\RED{Why ordering hazards affect system performance.}
