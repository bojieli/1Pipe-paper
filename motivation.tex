...
$A$ writes $O_1$ and $O_2$ in a message scattering, while $B$ reads $O_1$ and $O_2$ simultaneously. $B$ may get an inconsistent state where only one of $O_1$ and $O_2$ is written. IRIW hazard is common in shared data structures, where metadata and data needs to be accessed together atomically. Note that messages to $O_1$ and $O_2$ may be different, so it is a \textit{scattering} instead of multicast.
...
\RED{Define total order message scattering.}

We propose total order message scattering (TOMS), which ensures a group of messages to be scattered atomically in FIFO order. A message scattering means that a host $S$ scatters a group of (potentially different) messages $M_1, \ldots, M_n$ to receivers $R_1, \ldots, R_n$ respectively.

\textbf{Total Order.} For each pair of receivers $R_1$ and $R_2$ and
for each pair of messages $M_1$ and $M_2$ that are delivered to both receivers, $R_1$ is delivered $M_1$ before $M_2$ if and only if $R_j$ is delivered $M_x$ before $M_y$.~\cite{kshemkalyani2011distributed}

\textbf{FIFO.} For each pair of sender $S$ and receiver $R$ and for each pair of messages $M_x$ and $M_y$ that are delivered from $S$ to $R$, $M_x$ is delivered before $M_y$ if and only if $M_x$ is sent before $M_y$.


\RED{Discuss how TOMS solves the hazards above.}

\subsection{Application Scenarios}

\subsubsection{One-Shot Transactions}
...
Total order communication ensures that each replica receives an identical sequence of write operations from other replicas.
...
\subsubsection{Memory Consistency in DSM}
...
X86 ensures a \textit{total store ordering} (TSO) memory model~\cite{sewell2010x86} \RED{where each core observes a consistent ordering of writes from all other cores}.
