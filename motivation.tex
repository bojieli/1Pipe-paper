...
\centering
	...
	{\includegraphics[width=.2\textwidth,page=1]{images/ordering-cropped.pdf}}
    \hspace{0.05\textwidth}
    \subfloat[Independent read, independent write.\label{fig:ordering-iriw}]
	{\includegraphics[width=.15\textwidth,page=3]{images/ordering-cropped.pdf}}
    \hspace{0.05\textwidth}
	\subfloat[Independent multi-write.\label{fig:ordering-imw}]
	{\includegraphics[width=.2\textwidth,page=2]{images/ordering-cropped.pdf}}
...
Together with different delays of network paths, several categories of ordering hazards may take place, as shown in Figure~\ref{fig:ordering}. \textcolor{red}{Some references here?}.
...
A \textit{message scattering} is a group of (potentially different) messages $M_1, \ldots, M_n$ sent from a host to one or more hosts $R_1, \ldots, R_n$ respectively. Each host in the system may send message scatterings to any group of hosts.
...
\textbf{Validity.}
If the sender and all receivers $R_i$ of a message scattering $S$ have no failure, and the network has no permanent failure, each message $M_i \in S$ is eventually delivered by $R_i$. No message is delivered more than once.

\textbf{Atomicity.}
For any pair of messages $M_x$ and $M_y$ in a same message scattering, either $R_x$ deliver $M_x$ and $R_y$ deliver $M_y$, or $R_x$ does not deliver $M_x$ and $R_y$ does not deliver $M_y$.

\textbf{Total Order.}
There is a total order among message scatterings. For each pair of message scatterings $S_i$ and $S_j$ with at least one overlapping receiver $R$, $R$ is delivered message $M_i \in S_i$ before $M_j \in S_j$ if and only if $S_i$ is ordered before $S_j$.
...
\sys can solve all three hazards in Sec.\ref{subsec:tso}.
...
In IRIW and IMW hazards, messages $A \rightarrow O_1$ and $A \rightarrow O_2$ belong to a same scattering $S_1$, and $B \rightarrow O_1$ and $B \rightarrow O_2$ belong to another scattering $S_2$.
...
Another ubiquitous application of \sys is \textit{serializable log replication}~\cite{birman1985replication,petersen1997flexible,belaramani2006practi}.
...
Recent x86 processors provides a \textit{total store ordering} (TSO) memory model~\cite{sewell2010x86} in which each core observes a consistent ordering of writes from all other cores.
...
In addition, \sys enables an efficient implementation of \textit{memory barrier}.
...
Existing Approaches (centralized, timestamp). Not scalable or efficient. Primitive not provide atomic group of messages.
