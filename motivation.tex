...
\RED{Total order unicast, does not have the concept of multicast group.}

\RED{Return failure and let application retry if end-to-end flow control or congestion control does not allow. Keep in-network queues low to minimize reordering delay.}

\RED{messages can be multi-packet.}
...
\begin{tabular}{l}
	\hline
	TS = onepipe\_send(vec[<dst, msg>]) \\
	\hline
	\hline
	TS, vec[<src, msg>] = onepipe\_unreliable\_recv() \\
	\hline
	onepipe\_send\_fail\_callback(func(TS, dst, msg)) \\
	\hline
	\hline
	TS, vec[<src, msg>] = onepipe\_reliable\_recv() \\
	\hline
	...
	\hline
\end{tabular}
\caption{Programming API of \sys{}. Vec[<dst, msg>] indicates a batch of messages that have a same timestamp.}
\label{tab:abstraction}
\end{table}

As Table~\ref{tab:abstraction} shows, \sys{} provides two services with different reliability guarantees: first, \texttt{onepipe\_unreliable\_recv} is a \emph{best effort} service where packet loss is possible, and \sys{} guarantees that delivered messages satisfy causal and total order properties. \texttt{onepipe\_unreliable\_recv} returns all messages with a same timestamp as a batch.
...
The application should use \texttt{onepipe\_send\_fail\_callback} to detect lost packets and possible remote host failure.

Second, \texttt{onepipe\_reliable\_recv} is a \emph{reliable} service which in addition to ordering, guarantees reliability: \emph{a message is guaranteed to be delivered if both sender and receiver processes are correct}.
...
In addition, each process may register a callback function via \texttt{onepipe\_proc\_fail\_callback} to get notified of failed processes.
...
\RED{Traditional approaches require at least 2 RTTs, first for serialization, second for replication. cite Exploiting Commutativity For Practical Fast Replication.}
...
Each recipient processes messages from all DAOs in timestamp order\footnote{DAOs with a same timestamp are ordered by the ID of initiator.}, so, the DAOs are sequentially consistent.
...
If we do not need strict wall-clock ordering of read-only DAOs, as an optimization, we can use unreliable \sys{} for read-only DAOs and reliable \sys{} for other DAOs. If a read-only DAO fails due to packet loss, it can be retried without violating sequential consistency.

\RED{In terms of SNOW and NOCS theorem, 1 RTT for read-only TXN, throughput optimal, latency close to optimal (wait for barrier).}
...
\RED{General transactions with Opacity (cite FaRMv2): can remove waiting for clock uncertainty in read and write timestamps. Because \sys{} ensures locks of previous transaction are visible to new read or commit transactions.}
