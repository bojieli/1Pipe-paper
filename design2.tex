...
\revise{For example, if a host or the only network link from a host fails, all processes on it are disconnected from the data center network, so they are considered to fail simultaneously.
...
\parab{Controller Forwarding.} If a network failure affects connectivity between $S$ and $R$, the Commit phase in 2PC and the Recall step in failure handling may stall. %$S$ repeatedly retransmits a message but cannot receive ACK from $R$.
