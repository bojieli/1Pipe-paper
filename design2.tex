...
Now, we \revise{present the design reliable \sys{} that can handle packet loss and failure.}
...
For example, host $A$ sends to $B$, then sends to $C$ via a different path. The packet $A \rightarrow B$ is lost on the way. Then, $A$ crashes.~\wei{I think you want to say three events happen according to the following order: $A \rightarrow B$ is lost; $A \rightarrow C$ arrives; $A$ crashes} The packet $A \rightarrow C$ is delivered, while $A \rightarrow B$ cannot be recovered. The failure to deliver $A \rightarrow B$ and the delivery of $A \rightarrow C$ violate reliable ordering property.
...
\centering
	...
	\caption{Two Phase Commit in reliable \sys{}.~\wei{Can we use dashed lines for commit messages?}}
	\label{fig:2PC}
\end{figure}
...
Our key idea is a \emph{Two Phase Commit (2PC)} approach:~\wei{key idea to what? handle packet losses?}
