...
\parab{\revise{Limitations.}} \revise{If a process fails permanently, the last timestamp it has delivered cannot be known exactly, so, \sys{} only ensures all correct receivers and recovered receivers deliver messages consistently. In addition, when network partition occurs, separated receivers may deliver messages after failure timestamp. \sys{} relies on the application to coordinate such failures. We expect future work to improve failure handling and atomicity.}
