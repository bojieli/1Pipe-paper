...
A message $M$ from sender $S$ to receiver $R$ with timestamp $T$ should be delivered if and only if $S$ commits $T$ and does not recall $M$.
...
\begin{ecompact}
	...
	\item Receiver of another message in the same scattering fails before ACK in Prepare phase: discard according to Recall step.
	\item Receiver of another message in the same scattering fails after ACK in Prepare phase: deliver according to 2PC.
