...
Traditionally, in a network with arbitrary delays, messages are not garanteed to be delivered in a consistent order.
...
In this work, we seek a solution to provide efficient and scalable total-order communication for distributed systems in a data center environment.
\RED{Should we emphasize, focus on \textbf{intra-} data center?}
...
Consistency between metadata and data requires \RED{only requires message scattering, not total-order, should be 'requires a group of messages to be scattered atomically'} \textit{total-order message scattering} (\sys), which ensures a group of messages to be scattered atomically at the two sites.
...
We further merge barriers hierarchically at switches to reduce \REDBLU{communication overhead}{the amount of beacon traffic} (Sec.\ref{sec:ideal}).

For the second challenge, physical clock synchronization seems to be a straightforward solution.
However, it does not account for the network latency difference incurred by the OS and links, \REDBLU{thus makes}{making} fast links constantly waiting for slow links.
