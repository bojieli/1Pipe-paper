...
We implement \sys on network devices with different programming capabilities ($\S$\ref{sec:impl}). Sec.\ref{sec:p4} introduces the implementation on the state-of-the-art programmable switches~\textcolor{red}{cite Barefoot Tofino and Cavium NP} that supports flexible stateful per-packet processing. Sec.\ref{sec:commodity} presents the implementation on the fixed-function switches with a CPU~\textcolor{red}{cite some Arista switches}, which can process control packets but not each and every data packets.  
