...
To this end, we introduce a \emph{barrier} timestamp on each link and switch~\wei{'each link' should be enough here.}, which is essentially the \emph{lower bound} of the timestamps of all future arrival packets.
Each switch aggregates barrier information of all the ingress links to derive the barrier for all the egress links. In this way, barriers propagate in the DAG\wei{give full name of DAG here} network along with packet forwarding, and the receiver can deliver the messages with timestamps below the barrier in order.
...
Regarding packet losses and failures, \sys{} provides a \emph{best effort} service in which lost messages are not retransmitted; and a \emph{reliable} service in which a message is guaranteed to be delivered only if both sender and all receivers\wei{all the hosts?} in the scattering do not fail.
In addition, reliable \sys{} ensures \emph{restricted failure atomicity}: either all or none messages in a scattering are delivered unless a receiver fails permanently or network partitions after the messages are sent.~\wei{What are the relationships between this sentence and previous sentence.}
