...
\RED{no concept of CATOCS channel/group, one CATOCS channel}

\RED{novelty: unicast and destination-addressed multicast, not group multicast}

\sys{} can achieve distributed atomic multi-object read and write with a single batch of messages because they are delivered at the same logical time. Replication in \sys{} is simply a batch of messages. \sys{} also provides a total ordering of communication events, thus reducing fences and improving concurrency of distributed systems.
...
Regarding packet loss and failures, \sys{} provides a \emph{best effort} service in which each message is delivered at most once; and a \emph{reliable} service in which a message is guaranteed to be delivered if both sender and receiver are correct.
...
As case studies, first, \sys{} scales linearly for a transactional key-value store in both uniform and YCSB~\cite{cooper2010benchmarking} workload, whose throughput is 90\% of a non-transactional system (hardware limit) and outperforms FaRM~\cite{dragojevic2014farm} by 2$\sim$20x especially under high contention.
...
\RED{Contributions of this paper is as follows: ...}
