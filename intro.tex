...
Today's data centers host a variety of distributed systems\RED{add some references}. With arbitrary network delays, packet losses and failures, it is challenging to guarantee the reliability and ordering of message delivery. For example, multiple clients updates an object simultaneously in a distributed storage, where each client needs to update both metadata and data on different shards.
The shards storing metadata and data may receive the updates in different orders, and some shards may fail to receive the updates, thus violating data consistency
To address this problem, transactional systems often introduce synchronization overhead \RED{introduce complex mechanisms for synchronization?}and complicate distributed system design.
...
To address this, we propose Reliable Ordered Message Scattering (\sys), a group communication abstraction that provides both linearizable ordering and reliability.
\begin{ecompact}
\item \textbf{Message scattering}~\cite{kshemkalyani2011distributed} is a communication pattern where a host sends a group of (potentially different) messages to multiple hosts simultaneously .

\item \textbf{Linearizable ordering} means that each receiver delivers messages from different senders in the wall clock order of message sent time.
It provides \emph{total order}, ensuring that messages are delivered in the same order to all receivers.
This also implies \emph{FIFO}, i.e., if a message is sent after the other, it will also be delivered after the other.
Moreover, messages obey the \emph{causal order} in the Lamport logical clock sense~\cite{lamport1978time}.

\item \textbf{Reliability} means both (1) \emph{atomicity} or \emph{serializability}, i.e., either all or none receivers deliver messages in a scattering group, and (2) \emph{validity}, i.e., a scattering is guaranteed to be delivered if all the senders and receivers are correct, and the network is not partitioned.
\end{ecompact}
