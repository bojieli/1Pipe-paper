...
The lack of order in communication often complicates distributed system design.
...
In this work, we propose \sys{}, a communication primitive that provides a single FIFO pipe abstraction in a distributed system. As Figure~\ref{fig:1pipe} shows, messages are sent in batches and serialized in a virtual pipe, which enables different receivers to process messages from senders in a consistent order.
More precisely, \sys{} provides Causally and Totally Ordered Communication Support (CATOCS)~\cite{cheriton1994understanding}: (1) messages are \emph{totally ordered}, ensuring that they are delivered in the same order to all receivers. In addition, \emph{batching} in \sys{} enables grouping multiple messages at the same position of the total order. (2) Messages are delivered obeying the \emph{causal order} in the Lamport logic clock sense~\cite{lamport1978time}. (3) Messages between a pair of sender and receiver are in \emph{FIFO} order.

\sys{} can achieve distributed atomic multi-object read and write with a single batch of messages. Replication in \sys{} is simply a batch of messages. \sys{} also provides a total ordering of communication events, thus improving concurrency of distributed systems.
...
In this work, we propose a scalable and efficient implementation of \sys{} in a data center network, where the topology is regular~\cite{leiserson1985fat,greenberg2009vl2}, and switches have generally good programmability.
...
Considering packet loss and failures, \sys{} provides a \emph{best effort} service in which each message is delivered at most once; and a \emph{reliable} service in which a message is guaranteed to be delivered if both sender and receiver are correct.
In addition, the reliable service ensures \emph{restricted failure atomicity}: either all or none messages in a batch are delivered unless a receiver fails permanently or network partitions after the messages are sent.
The reliable service uses a two-phase commit (2PC) approach, where the first phase is end-to-end, and the second phase aggregates \emph{commit barriers} through the network.
It adds one network round-trip time (RTT) compared to the best effort service.
...
We evaluate the performance of \sys in a 32-server cluster.
\sys{} achieves linearly scalable throughput with 512 processes, achieving 5M messages per second per thread.
Best effort \sys{} adds less than 10$\mu$s delay to message delivery, while reliable \sys{} adds less than 21$\mu$s.
\sys{} has robust performance with packet loss, and can recover from failures in 50$\sim$500 $\mu$s.
\sys{} only adds 0.3\% network bandwidth overhead and a CPU core per switch for periodic beacons.
...
As a case study, \sys{} scales linearly for a transactional key-value store in both uniform and YCSB~\cite{cooper2010benchmarking} workload, whose throughput is 90\% of a non-transactional system (hardware limit) and outperforms FaRM~\cite{dragojevic2014farm} by 2-20x especially under contention.
The latency of \sys{} is consistently low.
\sys{} also scales linearly for TPC-C~\cite{tpcc} benchmark, which outperforms Lock and OCC by 10x. In addition, \sys{} is resilient to packet loss.
Finally, by removing fences and enabling replicas to serve reads, \sys{} improves remote data structure performance by 2-4x.
