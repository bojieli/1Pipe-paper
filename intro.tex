...
In normal cases, best effort \sys{} adds low delay to message delivery. Reliable \sys{} can deliver messages within \textbf{10 $\mu$s}.
...
As a case study, in YCSB+T~\cite{dey2014ycsbt} transactional key-value store, \sys achieves 6.4 million transactions per server and 40~$\mu$s latency for independent transactions, which is 10x more efficient than MVCC concurrency control and Paxos-based replication. The performance is almost the same as a non-transactional system.
Moreover, the performance scales linearly with number of servers.

\textbf{Need update evaluation results.}
