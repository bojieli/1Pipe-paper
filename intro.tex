...
We propose Reliable Ordered Message Scattering (\sys), a group communication abstraction that provides both real-time ordering and reliability.
...
\emph{Real-time ordering}, sometimes called \emph{linearizability}, means that each receiver delivers messages from different senders in the wall clock order of message sent time.
Real-time ordering provides the guarantee that messages are delivered obeying the \emph{causal order} in the Lamport logic clock sense~\cite{lamport1978time}.
...
\emph{Reliability} means that (1) \emph{atomicity} or \emph{serializability}, i.e., either all or none receivers deliver messages in a scattering, and (2) \emph{liveness}, i.e., a message is delivered exactly once if the sender and all receivers in the scattering is correct, and the network is not partitioned.
...
Most existing works achieve atomic broadcast for a small-to-medium group of known and trusted hosts, where each client sends the same message to every participant.
However, distributed systems in data centers may scale to thousands of shards~\cite{nishtala2013scaling}, where each client communicate with a small and non-predetermined subset of hosts for each scattering.
To this end, we design \sys for a very large group of unknown and untrusted hosts in data centers.
In this scenario, existing solutions suffer from scalability or efficiency limitations.

To achieve real-time ordering in \sys, one line of work leverages logically centralized coordination, \textit{e.g.}, centralized sequencers~\cite{eris}, or tokens to be passed among senders and receivers~\cite{rajagopalan1989token,kim1997total,ekwall2004token}.
...
To achieve reliability in \sys, we need to achieve consensus among receivers in the presence of failures.
Most existing works, e.g. Paxos~\cite{lamport1998part} and ZAB~\cite{hunt2010zookeeper}, use voting to guarantee that a quorum of receivers agree on the sequence of messages to be delivered.
Each vote needs to be broadcast to and collected from all hosts, incurring significant communication overhead for hundreds of thousands of hosts in a data center.
In addition, Paxos does not allow Byzantine failure of hosts, which is insecure for a data center service with potentially malicious hosts.
Achieving Byzantine fault tolerance~\cite{castro1999practical} in an asynchronous network is notably complicated and inefficient~\cite{mickens2014saddest}.
...
Our principle is to co-design end hosts with underlying data center networks to achieve synchrony and detect failures.
We synchronize the clocks on hosts and a timestamp is attached to each message by the sender.
...
We first consider the common case without packet loss and failures.
Each receiver needs to deliver messages in non-decreasing timestamp order.
...
A host can \emph{reliably deliver} a message when it is sure that all hosts have received messages with lower timestamps.
To achieve reliable delivery, a receiver responds an ACK to each tentatively delivered message, and the switch aggregate \emph{ACK barriers} hierarchically from receivers to senders (Sec.\ref{sec:ack-barrier}).

To recover from occasional packet loss or corruption, we take advantage of the fact that data centers are typically well engineered and have very low packet loss rates~\cite{ports2015designing}.
We detect packet loss via counters in network switches, and rely on end hosts to retransmit lost packets.
Hosts stop delivering messages according to loss-free and ACK barriers when loss is detected.
...
To ensure liveness, we rely on the centralized SDN controller to resume operation of a largest strongly connected network partition (Sec.\ref{network-failure}).
...
In normal cases, \sys can deliver messages reliably in two \emph{network diameters}, i.e. the maximum one-way delay between two hosts, which is theoretically optimal.
...
Making use of tentative delivery, \sys can execute externally consistent independent transactions in a round-trip time, the same as a non-transactional and non-replicated system.
As a case study, \sys achieves 6.4 million transactions per server and 40~$\mu$s latency for independent transactions in YCSB+T~\cite{dey2014ycsbt} transactional key-value store, which is 10x efficient than other systems and the performance scales linearly with number of servers.
