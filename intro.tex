...
For evaluation, we implement three incarnations of \sys on network devices with different programming capabilities. Sec.\ref{sec:p4} introduces the implementation on reconfigurable switching chips~\cite{tofino,cavium} that can support flexible stateful per-packet processing. Sec.\ref{sec:commodity} presents the implementation on the switch CPU. In case that switch vendors do not expose accesses to switch CPUs, sec.\ref{sec:end-host} presents the implementation on the host CPU. 
