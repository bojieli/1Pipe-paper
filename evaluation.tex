...
For experiments with 16$\sim$128 hosts, we use each physical server as 16 logical servers. Each logical server has a pair of dedicated cores, which maintain timestamps independently and communicate via separate RDMA queue pairs.
To evaluate datacenter-scale scalability ($\ge$256 hosts), we build a software simulator with random link one-way delays from $U(0\mu s,10\mu s)$.
