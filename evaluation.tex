...
\sys achieves throughput close to underlying hardware limit (8~M messages per second per core using RDMA), and scales linearly to 1K hosts.
...
In our settings, \sys consumes 0.1\% bandwidth on idle network links, and adds an 8-byte tag to each network packet. On switches, beacon packets can be processed by a CPU core in commodity switch. On hosts, reordering consumes less than 10\% of a core.
...
\sys is adaptive to delay variance in the network and the delay is optimized in one RTT. Consequently, 
