...
\sys{}/BE with \revise{programmable} chip delivers the lowest latency overhead compared to the unordered baseline. The average overhead (1.7$\sim$2.3$\mu$s) is almost constant with different number of network layers and processes, \revise{which is half of beacon interval plus average clock skew}. The tail latency overhead \revise{(1.7$\sim$3.3$\mu$s)} is \revise{half of beacon interval plus maximum clock skew}.
...
\revise{As Figure~\ref{fig:reorder-testbed} shows, reliable \sys{} adds an RTT to best effort \sys{}} due to Prepare phase in 2PC.
