...
%First, we evaluate the reordering delay, CPU overhead and network overhead of \sys under normal conditions. Then we deep dive into the behavior of \sys under abnormal conditions, including new node joining, failure, route change, packet loss and inter-datacenter topology.
...
\RED{In the evaluation about beacon overhead, we should not simply show a line chart of reordering delay and beacon overhead. We should show the impact on application performance with different reorder delays.}

\subsection{Efficiency}

\RED{
\begin{enumerate}
\item Compare with hierarchical merge using end hosts only (validate our principle of ordering/forwarding separation).
\item Fault tolerance, failure recovery and incremental deployment 
\item Reordering delay with background traffic (DCTCP + ECN) 
\item minimax clock synchronization (compare reordering delay with physical clock synchronization)
\end{enumerate}
}
