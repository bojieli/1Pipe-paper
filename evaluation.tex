...
\begin{figure*}[t]
        ...
        \vspace{-5pt}
        	...
        	\vspace{-5pt}
        	\caption{Reordering overhead on hosts.}
...
Clocks are synchronized via PTP~\cite{correll2005design} every 125 ms, achieving an average clock skew of 0.3~$\mu$s (1.0~$\mu$s at 99\%).
...
%In the extreme case of high loss rate, there is always some in-flight message that gets lost, so the latency is high.
