...
\sys{} scales linearly to 512 processes, achieving 5M \revise{64-byte} messages per second per process (\textit{i.e.}, 80M msg/s per host).
...
Figure~\ref{fig:reorder-testbed} shows the message delivery latency of \sys{} \revise{when the system is idle and thus have zero queuing delay}.
BE-\sys{} with \revise{programmable} chip delivers the lowest latency overhead compared to the unordered baseline. The average overhead (1.7$\sim$2.3$\mu$s) is almost constant with different number of network layers and processes, \revise{which is half of beacon interval plus average clock skew}. The tail latency overhead \revise{(1.7$\sim$3.3$\mu$s)} is \revise{half of beacon interval plus maximum clock skew}.
...
The RTT and host forwarding delay are proportional to network hop count\revise{, which is $\sim10 \mu$s for the 5-hop topology}.
...
\parab{\revise{Scalability to larger scale.}}
