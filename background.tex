...
A data center switch consists of a switching chip~\cite{broadcom,tofino} and a CPU (Figure~\ref{fig:switch}). The switch operating system~\cite{arista-eos} runs on the CPU to control the switching chip. % (\textit{e.g.}, configure chip registers).
...
supports flexible packet parsers and stateful registers. Users can program Tofino using P4~\cite{bosshart2014p4} to achieve \textit{customized stateful per-packet processing}.
