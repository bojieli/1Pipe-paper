...
Without going into details, we want to point out that one key property of the switch is that the queuing model ensures FIFO property for packets with the same ingress and egress port, 
...
supports flexible packet parsers, reconfigurable pipelines and stateful memory. Users can program Tofino using P4~\cite{bosshart2014p4} for \textit{flexible per-packet processing}.
