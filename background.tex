...
Our goal is to build a scalable total-order message scattering service in data centers. Such a service should satisfy the following requirements.
...
\parab{Scalable.}The system should be scalable with number of switches and hosts, which implies no centralized coordinator and the overhead should not increase too much with system scale.
...
\parab{Readily Deployable.}The service should be readily deployable using existing commodity switching hardware in data centers.
...
When a packet is received from an input link, it first goes through the ingress pipeline to determine the output link and queueing priority, then get buffered into the corresponding FIFO queue.
The egress pipeline pulls packets from queues according to priority, applies header modifications and sends them to output links.
...
If packets $P_1, P_2$ arrive in order to an input link of switch $S$, and they are routed to the same output link, then their egress ordering should also be $P_1, P_2$.
