...
We aim to build a reliable ordered message scattering service with good scalability and efficiency in data centers. Such a service should satisfy the following requirements.
...
First, the \textit{reordering delay}, which is the time interval beween message arrival and message processing, should be minimal. Second, the network and computation overhead of the system should be minimal. 

\parab{Scalable.}The system should be scalable with the number of switches and hosts, which implies no central coordinator and the overhead should not increase too much as the system scales out. \RED{can be improved}
...
\parab{Fault Tolerant.}The correctness of the system should not be violated under packet losses, or failures of switches, links or end hosts. %The system ensures liveness unless network partition occurs.

\parab{Readily Deployable.}The service should be able to be implemented using commodity data center switches.
...
This queueing model ensures FIFO property for packets with the same priority. In addition, packet corruption on input links and buffer overflow inside switches can be detected, and a packet loss counter is incremented per lost packet.

The data center switch can provide good programmability. First, the CPU can be used to co-process (a small amount of) data plane packets with high flexibility~\cite{lu2011serverswitch}.
...
supports flexible packet parsers, reconfigurable pipelines of match action tables and some stateful memory (\textit{e.g.}, counters and registers). Users can program Tofino using P4~\cite{bosshart2014p4} to achieve \textit{flexible per-packet processing}.
