...
Although the design above works on a P4 programmable switch, many commodity switching chips are \RED{capable or not? I guess here should be 'capable'} not capable of storing states $R_i$ and calculating the minimum among $R_i$.
...
\RED{we shouldn't and actually can't say when to leave the \sys and when not. I think

	There is another trade-off between beacon overhead and system rejoin latency. A sender can notify the switch its exit and stop sending beacons, but it must rejoin (need a RTT, see Sec.\ref{sec:failure}) before sending messages again.
...
When a host, switch or network link fails, the barrier timestamp would stall.
We detect failures using timeout and removes failed nodes to achieve fault tolerance.
Because failures can be detected and messages would not delay arbitrarily, achieving atomicity is much easier than consensus in general distributed systems.
