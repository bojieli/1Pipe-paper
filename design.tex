...
However, since different network paths have different propagation and queuing delays, \revise{this} approach will drop too many messages, \textit{e.g.}, 57\% in an experiment where 8 hosts send to one receiver.~\wei{add reference to the paragraph.}
...
\parab{Handling host and link failures.}
...
For a new host, its timestamp is synchronized to the maximum timestamp of its neighbors \revise{under the same rack}. After a link is added, because a packet on the new link may have a smaller barrier timestamp $R_i$ than the barrier $B_{new}$ of destination switch, the switch updates $B_{new}$ only if $R_i$ is larger than the original $B_{new}$.~\wei{I don't really understand above paragraph. It seems to me that new hosts just need to synchronize clocks with other hosts.}
