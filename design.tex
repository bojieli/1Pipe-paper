\section{\sys in a Reliable Network}
...
\textbf{Scalability}.
If there are $N$ senders and $N$ receivers, \REDBLU{the complexity of communication is $N^2$.}{it requires $N^2$ messages per beacon interval.}
...
\textbf{Packet loss}.
If a packet is lost, the receivers will observe \REDBLU{an incomplete ordering of events.}{different orderings of events.}
...
\REDBLU{Instead of buffering packets, each network switch only maintains ordering information (aka the control plane), while packets are forwarded as usual (aka the data plane).
...
The second is a \textit{barrier timestamp} field, which is initialized by end-hosts but will be modified by switches as the packets moving.
}
...
\emph{When a switch or an end host receives a packet with barrier timestamp $B$ from a network link $L$, it indicates that all future packets from link $L$ will have higher message \REDBLU{and}{or} barrier timestamps than $B$.}

To derive barriers, \REDBLU{senders assign the same value to both fields.
And}{} a network switch maintains a register $R_i$ per ingress port $i \in \mathcal{I}$, where $\mathcal{I}$ is the set of all ingress ports.
...
\REDBLU{One liveness requirement of the system is \textit{Loop-Free Routing}(Sec.\ref{sec:assumptions}).}
...
\REDBLU{. So we can't increase the barrier timestamp because a packet may be forwarded in this routing loop indefinitely.}
...
\section{\sys in an Unreliable Network}
\label{sec:reliable}

\RED{Leading paragraph: packet loss and node failure.}
...
\RED{Need to discuss atomic message scattering based on 2PC, improvement (fast commit when no loss), recalling messages on node failure.}

\RED{Discuss the difference of switch failure and host failure. Switch failure, sender can resend. Node failure, may need recall. Difference from consensus: all or nothing, quorum includes all nodes. Cannot recover from permanent host failure. Leave persistence, replication and failure recovery to the application.}
