...
If the transport between sender and receiver is FIFO, a receiver can easily figure out the barrier if it has received messages from all the senders: the barrier is the minimum timestamp of latest messages from \revise{all the senders}. Therefore, a naive solution would be for every sender to send periodic beacons~\wei{The reader has no idea of beacons now. We mention this in 4.2.} to every receiver so that the receivers can figure out the barrier and deliver messages. Unfortunately, this solution requires quadratic number of beacons, which does not scale.
...
\parab{Handling host and link failures.}~\wei{How about switch failures?}
...
For a new host, it synchronizes clock with the time master.
