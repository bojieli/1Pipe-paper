...
To \textit{clear} the loss encountered flag after all previous losses are recovered, as shown in Algorithm~\ref{alg:loss-detection}, when commit barrier catches up with unreliable barrier recorded at last packet loss, the switch clears loss encountered flag. At this time, messages below commit barrier are received and messages above that are not lost before passing through this switch.
