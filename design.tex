...
Best effort \sys{} provides total order, causal, and FIFO message delivery\revise{, \finalrevise{but does not retransmit lost packets and does not provide} atomicity}.
A naive approach \revise{to realize this} is ordering all messages with a centralized sequencer, \revise{which} would be a bottleneck. \revise{Instead, we will introduce how to achieve scalable ordering in the regular DCN topology.}
%~\wei{Maybe we should mention we assume network does not have any packet losses in this section.}


%We aim to preserve message routing topology and achieve scalable ordering in a distributed way.

%\RED{Strawman approach: only deliver received messages that are in order, and drop all out-of-order messages. Need to evaluate that this simple approach drops too many messages.}

%In this section, we design best effort \sys{}.
%The key idea is to aggregate the minimum message timestamp over the network links, and provide receiver processes with a timestamp lower bound of all future messages.
...
\sys sender assigns a non-decreasing timestamp for each message. Messages in a scattering have \finalrevise{the same} timestamp.
...
Message timestamp determines the delivery order \finalrevise{at a receiver}.
...
When a receiver delivers a message with timestamp $T$, it must be sure that it has received and delivered all the messages whose timestamps are smaller than $T$.
A \revise{straightforward} approach is to only deliver messages with non-decreasing timestamps, and drop all out-of-order messages.
However, since different network paths have different propagation and queuing delays, \revise{this} approach will drop too many messages, \textit{e.g.}, \revise{57\% received messages are out-of-order in our experiment where 8 hosts send to one receiver}. %~\wei{add reference to the paragraph.}
...
%Due to non-decreasing timestamp assignment,
If the transport between sender and receiver is FIFO, a receiver can easily figure out the barrier if it has received messages from all the senders: the barrier is the minimum timestamp of latest messages from \revise{all the senders}. Therefore, a naive solution would be for every sender to send \revise{timestamped messages} to every receiver so that the receivers can figure out the barrier and deliver messages. Unfortunately, this solution requires \revise{sending messages to receivers not in the scattering group}, which does not scale.
...
Notice that barrier aggregation only relies on hop-by-hop FIFO links instead of a FIFO end-to-end transport. \revise{Therefore, \sys{} can work with a variety of multi-path routing schemes~\cite{cao2013per,alizadeh2014conga,he2015presto,katta2016hula,zhang2017resilient,dixit2013impact,vanini2017let}.}

%so, the transport may utilize multipath and adaptive routing~\cite{glass1992turn}.
...
\parab{What is a beacon?} Unlike the message packet, the beacon packet only carries the barrier timestamp field and has no payload data. %thus minimizing bandwidth overhead. 
...
\revise{This hop-by-hop property ensures that the beacon overhead is unrelated to network scale.}
...
We introduce a beacon interval $T_{beacon}$. When a host or a switch output link does not observe any message packet for $T_{beacon}$ time, it generates a beacon packet. We should choose a suitable $T_{beacon}$ to \revise{balance bandwidth overhead and system delay}. 
\revise{The beacons are sent at synchronized times on different hosts, so that when network delays are identical, a switch receives beacons almost \finalrevise{simultaneously}. If beacons were sent on random times, the switch must wait for the last beacon to come, which increases expected delay of a message by nearly one beacon interval. With synchronized beacons, the expected delay overhead is only half \finalrevise{of the beacon interval}.}

\parab{\revise{Handling failures.}} %~\wei{How about switch failures?}
When a \revise{host, link, or switch} fails, the barrier timestamp of its neighbors \finalrevise{stop increasing}. 
...
\revise{This failure handling mechanism is decentralized.}

\parab{Addition of new hosts and links.}
\revise{For a new host, it synchronizes clock with the time master. When a link is added between a host and switch or between two switches, because the switch must maintain monotonicity of its $B_{new}$, it suspends updates to $B_{new}$ until the barrier received from the new link is greater than $B_{new}$.}
%For a new host, its timestamp is synchronized to the maximum timestamp of its neighbors \revise{under the same rack}. After a link is added, because a packet on the new link may have a smaller barrier timestamp $R_i$ than the barrier $B_{new}$ of destination switch, the switch updates $B_{new}$ only if $R_i$ is larger than the original $B_{new}$.
%~\wei{I don't really understand above paragraph. It seems to me that new hosts just need to synchronize clocks with other hosts.}
...
%On one hand, we hope to timestamp the events so that if two packets originated from two distant events arrive at a receiver simultaneously, these two events should have \finalrevise{the same} timestamp.
...
However, this approach does not take into account the delay variance of different network paths. Delay variance is caused by the variance of processing delays in OS kernel, queuing delays at the switch and different number \finalrevise{of the network} hops. The delay variance causes straggler problem. Consider two independent messages being sent from two senders to the same receiver at the same physical time. They might arrive at different times due to different network delays. As a result, the earlier message must wait for the later one to be processed together, which increases reordering delay.
...
%However, delay variance \finalrevise{of the network} paths makes physical time a suboptimal choice for timestamps.
...
In this section, we first handle crash failure of hosts and discuss failure recovery, then detect Byzantine failure of hosts, finally handle crash failure \finalrevise{of the network} switches and links.
...
To detect (c), ToR stores the sequence number, timestamp, message receiver and scattering group of the last message, and checks such fields in a new message if it belongs to \finalrevise{the same} scattering.
...
\parab{Problem.}
Failure \finalrevise{of the network} switches and links will stall barriers.
...
Two common cases \finalrevise{of the network} partition are resumable: failure of a ToR switch that isolates a rack from the rest of data center, and failure of all bridge links between two clusters.
...
Next, we deal with failure \finalrevise{of the network} switches or links.
...
In the extreme case \finalrevise{of the network} partition, all cross-partition scatterings will fail and scatterings within a partition still proceed with total order.
...
Because the routes in each shard originate from \finalrevise{the same} DC, they form a DAG and are loop-free by our definition.\RED{(confused, loop-free logical shard leads to what?)}
