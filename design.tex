...
Sender assigns an identical timestamp to each message in a scattering, namely $T_R$ and $T_W$.
Timestamps in different scatterings increase monotonically.
...
$\mathcal{I}_E$ is predetermined at route configuration time. \textcolor{red}{We need one or two sentences to summarize the insight of above mechanims.}
%Note that the switches do not read or modify message timestamps. 
In Appendix~\ref{appx:hierarchical_merge}, we prove that the hierarchical merge~\textcolor{red}{I don't see hierachical meaning here. It is just a per-hop update} described in (\ref{equ:derive_barriers}) and (\ref{equ:derive_barriers1}) maintains the property of barrier timestamps.
