...
\parab{Solution.}We aim to deliver messages in one-way network delay when no packet loss happpens, and fall back to a protocol similar to two-phase commit (2PC) with three one-way delays when losses happen. To achieve this, \sys levergaes NICs and switches to detect packet losses and hosts to locate and retrnamist loast packets via TCP-like loss recovery.
