...
\parab{Barrier timestamp.}However, it is challenging to proceess messages as described above. When a receiver wants to process a message with the timestamp $t$, it must guaranteee that it has received all the messages whose message timestamps are smaller than $t$. To this end, we introduce \emph{barrier timestamp}, which is the minimum message timestamp of all future arrival messages. Each receiver can 

process the messages whose message timestamps are smaller than the barrier timestamp value.

We use a simple example to show how to derive the barrier timestamp. 

\parab{Hierahical aggregation of barrier timestamp.}
...
%As shown above, barrier timestamp can be derived with all-to-all communication.
...
To this end, we send beacon packets in a per-\RED{idle-}link style.
The beacon packet can be sent by both the host and the switch.
But the destination must be \REDBLU{its one-hop neighbors}{neighbor node of the source.}
\REDBLU{For the beacon generated}{When the beacon is generated} by the host, the barrier timestamp field is initialized \REDBLU{in the same way}{same} as the regular message packet.
\REDBLU{For the beacon generated}{When the beacon is generated} by the switch, the barrier timestamp field is initialized according to Equation \ref{equ:derive_barriers} and \ref{equ:derive_barriers1}.
