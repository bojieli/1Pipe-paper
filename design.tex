...
In \sys, the receiver ACK\RED{send ACKs?} to the sender when it receives a message. Such a message is buffered but not delivered. Unlike regular 2PC, because \sys can only deliver a message after delivering all messages with lower timestamps, instead of multicasts commit messages, the sender sends \textit{update delivery barrier} messages. \sys hierarchically aggregate the updated delivery barriers in network so that the receivers know the timestamp lower bound of all delivered (and prepared) messages and can deliver the prepared messages accordingly.\RED{Rewrite}

When delivery barrier catches up with unreliable barrier recorded at last packet loss, the switch clears loss encountered flag, thus enabling receivers to deliver messages without waiting for an additional RTT.
