...
Commit barrier carries accumulative commit timestamp from the sender, which means that all messages sent by the sender and below commit barrier has been delivered.
...
When packet loss is encountered, we use a protocol similar to 2PC to update the commit barrier. 
...
Different from regular 2PC, because \sys can only deliver a message after delivering all messages with lower timestamps, the sender send a \textit{commit barriers} update message instead of multicast commit messages. \sys hierarchically aggregate the commit barriers in network so that the receivers know the timestamp lower bound of all delivered messages.

When commit barrier catches up with unreliable barrier recorded at last packet loss, the switch clears loss encountered flag, thus enables receivers to deliver messages without waiting for an additional RTT.
At this time, messages below commit barrier are received and messages above that are not lost before passing through this switch. This is shown in Algorithm~\ref{alg:loss-detection}, 
