...
\caption{Hierarchical aggregation of barrier timestamp. \RED{Change logos smaller. Use computer logo instead of this logo. Make the network links vertical (orthogonal to horizontal)}}
\label{fig:hierarchical_merge}
\vspace{-0.9em}
\end{figure}

\parab{In-order processing based on message timestamp.}\sys assigns the explicit timestamp for each message. When a sender scatters a group of messages, it assigns an identical timestamp for all the messages. Each sender assigns monotonically increasing message timestamps, \textit{e.g.}, synchronized physical time, to different scatterings. Message timestamp determines the processing order at receiver. To achive reliable order message scattering, all the receivers should process arrival meesages in ascending order of message timestamps.

\parab{Barrier timestamp.}However, it is challenging to proceess messages as described above. When a receiver wants to process a message with the timestamp $t$, it must guaranteee that it has received all the messages whose message timestamps are smaller than $t$. To this end, we introduce \emph{barrier timestamp, which is the lower bound of message timestamps of all future arrival messages}. Each receiver can maintain its own barrier timestamp and process the messages whose timestamps are smaller than the barrier timestamp value.
...
\parab{Hierahical aggregation of barrier timestamp.}As shown above, barrier timestamp can be derived with all-to-all communication. However, this approach does not scale well in production data centers. To this end, \sys leverages programmable switches to \emph{aggregate} the barrier timestamp information, thus forming the "control plane". Given the limited switch buffer resource, \sys does not reorder messages in the network. Instead, on the "data plane", \sys forwards messages in the network as usual, but reorders and processes them at the receiver side based on the barrier timestamp information provided by the switch.
...
But the destination must be its one-hop neighbors. For the beacon generated by the host, the barrier timestamp field is initialized in the same way as the regular message packet. For the beacon generated by the switch, the barrier timestamp field is initialized according to Equation \ref{equ:derive_barriers} and \ref{equ:derive_barriers1}.
...
\parab{Logical time.}Realizing the limitation of physical time, we use \emph{logical time} as the message timestamp. Logical time is essentially the estimated message arrival time, \emph{e.g.}, physical time plus one way delay from the sender to the receiver. 
\RED{We represent logical time as an offset of senders' local physical time.}
...
Note that we can \RED{use logical time to} measure the round-trip time ($RTT_i = d_i + r_i$) between two nodes, but one-way delay is hard to measure.
...
We \REDBLU{assume}{consider} physical clocks precise enough to measure short intervals \RED{Should we give a certain number here? e.g. 100us}.
...
Because timestamps can only increase monotonically, when the newly synchronized \REDBLU{logical clock}{offset $d_i$} is less than the current \REDBLU{one}{offset of $S_i$}, it has to ``stall'' the logical clock rather than \REDBLU{decrease}{decreasing} it.
During stall, the logical clock increases by 1 per scattering, while the synchronized \REDBLU{one}{offset} increases at the speed of physical clock.
...
In this regard, we add a \textit{forward pass} in which \REDBLU{$R_j$ receives and synchronizes clocks from $S_i$}{$S_i$ send to $R_j$ and synchronize clocks of $R_j$}, then run the \textit{backward pass} \REDBLU{likewise}{above} from $R_j$ to $S_i$.
In both forward pass and backward pass, each node receives potentially different \REDBLU{logical clocks}{offsets} from multiple peers, so we need two aggregation functions to resolve conflicts, namely \texttt{FwdFunc} and \texttt{BackFunc}.
