...
\textcolor{red}{The following paragraph looks weird. What information do you want to convey.}
...
%In the Lamport-style~\cite{lamport1978time} approach above, with $N$ senders and $N$ receivers, the complexity of communication is $O(N^2)$.\textcolor{red}{No relationship between two sentences.}
%Our solution is to separate the control plane from the data plane.
...
At its core, \sys separates the bookeeping of order information from message forwarding. \sys leverages programmable switches to provide the order information, which forms the ``control plane''. On the ``data plane'', given the limited buffer resource at the switch, \sys does not reorder messages in the network. Instead, \sys reorders messages at the receiver-end based on the order information provided by the switch.

To provide the order information, we attach two timestamp fields to each packet. The first one is \textit{message timestamp} field, which is set by the sender and will not be modified by the switch. \textcolor{red}{We need a sentence to explain the meaning of this field} The second one is \textit{barrier timestamp} field, which is initialized by the sender but will be modified by switches along the network path. The property of the barrier timestamp is:
