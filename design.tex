...
In this section, we first handle crash failure of hosts and discuss failure recovery of hosts, then extend it to detect Byzantine hosts, finally handle crash failure of network switches and links.
...
To this end, we need to ensure correctness and liveness even when the hosts are Byzantine~\cite{lamport1982byzantine}.

\parab{Solution.}
We use the ToR switch to monitor and enforce that hosts comply with the \sys protocol.
We trust network switches because they are centrally managed by data center operators via an isolated network.
For correctness, the ToR switch should detect the following cases: (a) incorrect source host ID, (b) timestamps and barriers that violate ordering properties, (c) inconsistent receiver list in a scattering, (d) retransmitted message is different from the initially sent one, and (e) send delivery barrier before receiving all ACKs.
Cases (a) and (b) can be detected by simple checks on the ToR.
To detect case (c), the ToR stores the sequence number, timestamp, message receiver and scattering receiver list in last message, and checks such fields in a new message if the timestamp is the same.
When the ToR detects a missing sequence number, forwarding is stalled and the sender is notified to retransmit the lost message.

Cases (d) and (e) are much harder to detect.
A naive approach requires the ToR switch to store the set of unique unacknowledged messages, which is impractical because the switch has limited storage capacity. % and per-packet computation needs to be O(1).
Now we propose a practical solution using constant storage on ToR.
...
The ToR signs each initially sent message and sets a \emph{signature} field in the message.%: msg.signature = signature(msg.timestamp, msg.sender, msg.receiver, msg.payload).
The ToR maintains a \emph{per-sender XOR sum} of signatures of \emph{initially sent messages}.
...
When the sender needs to retransmit the message, it attaches the signature, so the ToR can verify that the retransmitted message is identical to the initially sent one.
If one sigreturn message is lost, the sender can compute the lost signature according to the XOR sum.
If more consecutive sigreturn messages are lost, the sender retransmits such initially sent messages along with the previous and next sigreturn messages, so that the ToR can verify that the signatures agree with the XOR sums, then return the lost signatures.

The receiver responds ACK with the message signature.
When the ToR receives an ACK, it adds another signature to the ACK to verify its authenticity.
When the sender updates the delivery barrier to the ToR, it piggybacks signed ACKs of the messages to deliver.
If a receiver fails, the sender sends discarded messages and their signatures to the ToR.
The ToR maintains a per-sender XOR sum of signatures of \emph{messages to deliver}.
If all sent messages are either acknowledged or discarded, the XOR sum of signatures of initially sent messages would equal to that of messages to deliver.
However, with in-flight messages, the XOR sums will never equal.
To this end, the ToR maintains an array of XOR sums and tracks the two XOR sums for messages in each timestamp interval.
When the XOR sums of a timestamp interval equal, all messages in this interval are acknowledged or discarded, so the ToR sends the delivery barrier of the interval.
...
For (1), we introduce a \emph{barrier update timeout} to announce failure of a host when the barrier is kept the same during the timeout.%, which is set to three host failure timeouts.
For (2), we block a message if its timestamp is too far away from the ToR clock.
For (3), we enable a sender to trigger death notice of a receiver if it does not reply ACKs after a certain number of retransmissions.
...
Third, if the receiver does not reply after a certain number of retransmissions, the ToR announces death of the receiver.
Finally, the sender continues incrementing barriers after receiving the death notice.



\subsection{Fault Tolerance of Network}
