...
\subsection{Key Idea: Timestamp Barriers}
\label{sec:ideal}
...
As shown in Figure~\ref{fig:barrier}, host $R$ scatters $R_a$ and $R_b$ to shard($a$), host $W$ scatters $W_a$ and $W_b$ to shard($b$).
Each sender assigns an identical timestamp to each message in a scattering, namely $T_R$ and $T_W$.
Each sender assigns monotonically increasing timestamps to different scatterings.
Message timestamp determines the delivery order at receiver.
When shard($a$) receives both $R_a$ and $W_a$, due to monotonic timestamp assignment and the FIFO property between each pair of sender and receiver, shard($a$) knows that it will no longer receive timestamp $T \le T_R$ from $R$ or $T \le T_W$ from $W$.
Combined, it will no longer receive timestamp $T \le \min(T_R, T_W)$ from either $R$ or $W$, so it can deliver messages with $T \le \min(T_R, T_W)$.
In this way, each receiver derives a \textit{barrier timestamp}, which is the minimum message timestamp of all possible senders.

Barrier timestamp can be derived with all-to-all communication, but it does not scale. \sys leverages programmable switches to provide the order information, which forms the ``control plane''. On the ``data plane'', given the limited buffer resource at the switch, \sys does not reorder messages in the network. Instead, \sys reorders messages at the receiver-end based on the order information provided by the switch.

To provide the order information, we attach two timestamp fields to each packet.
The first one is \textit{message timestamp} field, which is set by the sender and will not be modified by the switch.
The second one is \textit{barrier timestamp} field, which is initialized by the sender but will be modified by switches along the network path. The property of the barrier timestamp is:

\emph{When a switch or a host receives a packet with barrier timestamp $B$ from a network link $L$, it indicates that all future packets arrival from link $L$ will have higher message and barrier timestamps than $B$.}

%To derive barriers, senders assign the same value to both fields.
To derive barriers, the sender initializes the both fields of a departure message using the same value. The switch maintains a register $R_i$ for each ingress port $i \in \mathcal{I}$, where $\mathcal{I}$ is the set of all ingress ports.
...
$\mathcal{I}_E$ is predetermined at route configuration time.
Using (\ref{equ:derive_barriers}) and (\ref{equ:derive_barriers1}), each switch independently derives the barrier timestamp based on all its input links.
As shown in Figure~\ref{fig:hierarchical_merge}, barrier timestamps aggregate hierarchically through layers of switches, and finally the receiver host gets the barrier of all reachable hosts and links in the network.
In Appendix~\ref{appx:hierarchical_merge}, we prove that this algorithm maintains the property of barrier timestamps.


When then receiver receives a packet with barrier timestamp $B$, it first buffers the packet in a priority queue which sorts packets based on message timestamps. Based on the property of barrier timestamp, the receiver knows that the message timestamp of all future arrival packets will be larger than $B$. Hence, it delivers all buffered packets with the message timestamp below $B$ to the application for processing.
