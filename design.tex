...
We introduce a beacon interval $T_{beacon}$. When a host or a switch output link does not observe any message packet for $T_{beacon}$ time, it generates a beacon packet. We should choose a suitable $T_{beacon}$ to \revise{balance bandwidth overhead and system delay}. %We set $T_{beacon}$ to 10 $\mu$s by default.
