...
\parab{Hierarchical barrier timestamp aggregation.} \sys exploits knowledge of the queuing structure of the network, making the lower bound aggregation much more scalable and efficient than if it was implemented at only a logical level.
...
We introduce a beacon interval $T_{beacon}$. When a host or a switch output link does not observe any message packet for $T_{beacon}$ time, it generates a beacon packet. We should choose a suitable $T_{beacon}$ to balance the trade off between bandwidth and delay overhead. %We set $T_{beacon}$ to 10 $\mu$s by default.
