...
\RED{Figure 12 should not compare directly with TAPIR/Eris. Should compare with lock-based, OCC, end-host based hierarchical merge and single-switch timestamp assignment (Eris).}
...
\RED{Evaluate average number of history rewriting for TOMS and ``last-writer-wins rule'', under different number of concurrent write sites.
(TOMS has at most rewrite history once at a write origin site; last-writer-wins rule may rewrite history at most N-1 times at any site.)}
