...
\sys{} provides a best effort service that delivers each message at most once, and a reliable service that guarantees delivery as long as both sender and receiver are correct. Reliable \sys{} also provides restricted atomic delivery for a group of messages, which uses a two-phase commit approach.

Evaluation on a 32-server testbed shows that \sys achieves scalable throughput and low latency with little CPU and network overhead.
As case studies, \sys{} achieves linearly scalable throughput and consistently low latency in transactional key-value store and TPC-C that outperforms traditional design by 2-20x, as well accelerating remote data structures.
