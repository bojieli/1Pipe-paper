...
To achieve ordering, existing approaches either employ centralized sequencers or tokens, thus suffer from limited scalability, or use distributed consensus protocols, which incurs high overhead in bandwidth and delay.

In this paper, we propose Reliable Ordered Message Scattering (\sys), an efficient and scalable method to scatter groups of messages in serializable order via data center network.
\sys is scalable
%and fault tolerant 
...
We build two \sys prototypes using Barefoot and Arista switches. Our evaluation shows that \sys achieves high performance and fault tolerance with low CPU and network overheads.
