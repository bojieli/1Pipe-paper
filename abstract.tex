...
The ability to have total ordering and atomic delivery of a group of messages can facilitate and simplify many distributed applications.
To achieve total ordering, existing approaches either employ centralized sequencers or tokens, thus suffer from limited scalability, or use distributed consensus protocols, which incurs high overhead in bandwidth and delay.
%\RED{we can't get rid of those overheads either, should emphasize on high overheads} bandwidth and latency overheads.

In this paper, we propose Total-Order Message Scattering (TOMS), an efficient and scalable method to scatter groups of messages in total order via data center network.
...
\sys is scalable and fault tolerant as it distributes work to each switch and end host.
...
We implement two \sys prototypes using P4 programmable switches and commodity switches.
